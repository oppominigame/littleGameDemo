...
@ccclass
export default class InsertAd extends cc.Component {
  @property(cc.Button)
  ...
  start() {
    this.createBtn.on(cc.Node.EventType.TOUCH_START, () => {
      ...
      this.log(`创建插屏广告, adUnitId：114187`)
    })
    ...
    this.onLoadBtn.on(cc.Node.EventType.TOUCH_END, () => {
      this.log('设置广告加载回调')
      // 设置 banner 成功加载回调
      this.insertAd.onLoad(onLoadCallBack)
    })

    this.offLoadBtn.on(cc.Node.EventType.TOUCH_END, () => {
      this.log('移除广告加载回调')
      // 设置 banner 成功加载回调
      this.insertAd.offLoad(onLoadCallBack)
    })
    ...
    this.onErrorBtn.on(cc.Node.EventType.TOUCH_END, () => {
      this.log('设置广告失败回调')
      // 设置 banner 成功加载回调
      this.insertAd.onError(onErrorCallback)
    })

    this.offErrorBtn.on(cc.Node.EventType.TOUCH_END, () => {
      this.log('移除广告失败回调')
      // 设置 banner 成功加载回调
      this.insertAd.offError(onErrorCallback)
    })
    ...
    this.onCloseBtn.on(cc.Node.EventType.TOUCH_END, () => {
      ...
      this.insertAd.onClose(onCloseCallback)
    })
    ...
    this.offCloseBtn.on(cc.Node.EventType.TOUCH_END, () => {
      ...
      // 设置 banner 成功加载回调
      this.insertAd.offClose(onCloseCallback)
    })
  }
